<script lang="ts">
  ...
  let { deploy }: DeployPipelineProps = $props();

  const currentIndex = $derived(getStageIndex(deploy.stage));
...
<div>
  ...
  >
    {#each DEPLOY_STAGES as stage (stage)}
      <li aria-current={stage === deploy.stage ? "step" : undefined}>
        <div
