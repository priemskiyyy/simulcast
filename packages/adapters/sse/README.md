...
A Simulcast channel is one `EventSource` URL. `url` builds the stream URL for a channel. Plain `message` events become publications without an `event`; names listed in `events` are delivered with that name, because `EventSource` has no wildcard. `data` is the event's string payload and `native` the `MessageEvent`.
