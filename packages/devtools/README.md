...
The same component ships under `simulcast-devtools/vue` and `simulcast-devtools/solid`; Svelte gets a `createDevtools()` attachment under `simulcast-devtools/svelte`. Without a framework, `new SimulcastDevtools({ client })` from the package root mounts into any element and follows a replacement client through `setClient`. Use your bundler's development flag; the examples use Vite.
